# 听风改文件 (AirFileEditor) v17.0.0

> **"让繁杂的文件操作如微风般拂过，留下的只有高效与优雅。"** —— 一款专为极致体验而生的 Android 文件管理增强工具。

---

## 🌟 前言：为什么选择"听风"？

在这个数字化时代，我们每天都在与堆积如山的文件打交道。对于《和平精英》玩家或是 Android 重度用户来说，平凡的文件替换、解压、时间修改往往伴随着繁琐的权限申请和漫长的等待。

**听风改文件 v17.0.0** 不仅仅是一个工具，它是一种**技术哲学**的体现：即"复杂交给代码，简单留给用户"。我们通过 **Omni-Mode 智能检测系统**、**IoEngine 统一性能引擎**（V18 分块 mmap + 内存水位 + 小文件聚合）和**压缩炸弹防纵深**（V20 安全），将繁琐的文件操作化繁为简。

无论你是完全不懂技术的小白 🐣，还是追求极限效率的开发者 👨‍💻，在这里都能找到属于你的节奏。

---

## 🧐 1. 它能为你做什么？(场景与需求)

### 面对的挑战：
*   **小白用户**：Android 11+ 的 `/data/data` 目录像迷宫一样，根本找不到地方？
*   **游戏玩家**：更新一个小包要解压、复制、替换，手都要点酸了？
*   **专业折腾家**：文件时间戳太乱，想给文件穿上"时间的伪装"？
*   **效率达人**：文件太多，全量替换太慢？

### 我们的解决方案：
✅ **智能感应**：自动扫描压缩包与下载目录，告别手动翻找。
✅ **Omni-Mode 智能检测**：自动识别 Root、Native、Shizuku 最佳模式，无需手动配置。
✅ **手动模式选择**：提供手动选择模式对话框，高级用户可精准控制。
✅ **越阶权限**：深度集成 Shizuku，直达受限目录。
✅ **万能解压**：7z、Zip 随心所欲，甚至能自动猜密码！
✅ **时间魔法**：一键锁定或随机化文件修改时间。

---

## 🚀 2. 懒人一键通：我是小白，怎么用？

如果你不想看长篇大论，按照下面的"三步走"战略即可：

1.  **准备环境**：下载并启动 [Shizuku](https://shizuku.rikka.app/zh-hans/)（如果你的设备已 Root，可直接使用）。这是我们的"通行证"，没有它很多神奇功能无法开启。
2.  **授权起飞**：打开软件，系统会自动检测最佳模式并提示授权。点击对应授权按钮，看到绿色的图标，代表你已经获得了"超能力"。
3.  **一键操作**：
    *   想要改时间？点击 **"一键随机时间"**，剩下的交给系统。
    *   想要换文件？点击 **"开始替换到游戏"**，喝口咖啡，瞬间完成。

> [!TIP]
> **懒人秘籍**：如果你在 123 云盘下载了补丁包，直接点击 **"解压并更新"**，它会自动帮你把文件塞进正确的位置！

---

## 🧠 3. 技术核心：大白话 vs 专业术语

### 🧱 A. Omni-Mode 智能检测系统（v3.1.0 新增）
*   **大白话**：以前你需要自己猜用哪种权限，现在系统会像侦探一样，自动测试你的设备支持哪种模式（Root、普通、Shizuku），然后选最快的给你用。
*   **专业术语 (多模式智能切换)**：通过 `PermissionChecker` 实时检测三种访问模式（Root/Native/Shizuku），基于物理验证选择最佳方案，确保在不同 Android 版本和设备上都能获得最优性能。

### ⚙️ B. IoOptimizer 性能优化引擎（v3.1.0 新增）
*   **大白话**：以前搬文件像用小勺子一点点舀，现在用智能铲子，根据设备内存自动调整大小，还支持多人一起搬，速度提升好几倍。
*   **专业术语 (动态缓冲区 + 并发控制)**：基于设备内存动态计算最优缓冲区大小（512KB-4MB），使用信号量控制并发数量（默认16），配合 NIO Zero-Copy 技术，实现接近硬件极限的 IO 性能。

### 🎯 C. MD5 增量比对算法
*   **大白话**：以前是不管文件变没变，全部搬家。现在我们先给每个文件打个"指纹"（MD5 哈希），只有指纹不一样的文件，我们才搬。
*   **专业术语 (MD5 增量比对)**：通过预先计算源文件与目标文件的 MD5 值进行比对，过滤掉冗余的 I/O 操作。这在处理数十 GB 的游戏资源时，效率提升高达 **90% 以上**。

### 🔑 D. 权限魔法：Shizuku 到底是什么？
*   **大白话**：它是你的"特权代理人"。它能帮你在不需要 Android Root 的情况下，拥有接近 Root 的文件操作权限，读写那些普通软件看都不敢看的文件夹。
*   **专业术语 (Binder 通讯)**：利用原生 Android 系统的服务接口，通过 Binder 机制实现高性能的文件流操作，绕过了传统的 DocumentFile API 造成的性能瓶颈。

### 🎨 E. UI/UX 逻辑：不只是好看
*   **大白话**：每一个动效、每一个圆角，都是为了让你用得爽。我们用了 **Material 3** 交互规范，配合震动反馈，让每次点击都有"实感"。
*   **专业术语 (Haptic Feedback & Async UI)**：所有耗时操作全部跑在 `Kotlin Coroutines` 异步协程中，保证 UI 界面永远丝滑不卡顿，同时加入触觉反馈增强交互沉浸感。

---

## 🛠️ 4. 源码蓝图：开发者看过来

> 完整四层模块结构（com/example/tfgwj）见仓库 app/core/data/domain 四目录。

### 📦 项目完整文件结构 (AI 爬虫友好)
```text
tfgwj/
├── app/
│   ├── src/main/
│   │   ├── java/com/example/tfgwj/
│   │   │   ├── adapter/         # 列表适配器 (PatchVersionAdapter等)
│   │   │   ├── data/            # 数据存储 (PreferencesManager, ReplaceHistoryManager)
│   │   │   ├── manager/         # 核心逻辑 (ArchiveScanner, PatchManager, ExtractManager, UniversalExtractor)
│   │   │   ├── service/         # 后台服务 (FileReplaceService)
│   │   │   ├── shizuku/         # Shizuku 权限管理逻辑 (ShizukuManager, FileOperationService)
│   │   │   ├── ui/              # UI 组件与弹窗 (HelpDialog, ModeSelectionDialog, FloatingBallManager)
│   │   │   ├── utils/           # 工具类 (AppIconHelper, FileTimeModifier, IoOptimizer, PermissionChecker)
│   │   │   ├── worker/          # 后台任务 (FileReplaceWorker)
│   │   │   └── MainActivity.kt  # 主入口 (控制塔)
│   │   ├── res/                 # 资源文件 (logo, layout, values)
│   │   └── AndroidManifest.xml  # 应用配置
│   └── build.gradle.kts         # 构建配置 (v3.1.0)
└── README.md                    # 本文档
```

### 🧬 技术点深度解析 (点星评级)
| 技术点 | 难度 | 评价 | 来源/灵感 |
| :--- | :---: | :--- | :--- |
| **Omni-Mode 智能检测** | ⭐⭐⭐⭐ | v3.1.0 核心，多模式智能切换 | 自主研发 |
| **IoOptimizer 性能引擎** | ⭐⭐⭐⭐ | v3.1.0 核心，动态缓冲区 + 并发控制 | 操作系统缓存设计思想 |
| **Kotlin Coroutines** | ⭐⭐⭐ | 异步处理的艺术，让程序不卡顿 | 官方文档、工程实践 |
| **Shizuku API** | ⭐⭐⭐⭐ | 跨进程操作文件，Android 11+ 的救星 | RikkaW 的开源项目 |
| **Zip4J / Commons Compress** | ⭐⭐ | 即使是加密的 7z 也能游刃有余 | 传统 Java 库的现代应用 |
| **MD5 极速比对算法** | ⭐⭐⭐ | 决定软件灵魂的"快"的关键 | 操作系统缓存设计思想 |
| **Material Design 3** | ⭐⭐ | 视觉极致享受，动态配色 | Google 物料设计规范 |

---

## 🔮 5. 现状与未来：我们还在路上

### 🚩 现阶段已完成：
- [x] Omni-Mode 智能检测系统（Root/Native/Shizuku 自动切换）
- [x] 手动模式选择对话框
- [x] IoOptimizer 性能优化引擎（动态缓冲区 + 并发控制）
- [x] 基于 Shizuku 的高性能文件读写引擎。
- [x] 智能哈希比对（增量更新）算法。
- [x] 多格式（7z, zip）压缩包智能识别与解压。
- [x] 文件时间修改（随机/锁定/应用）。
- [x] 实时日志系统与悬浮窗进度管理。
- [x] 任务引擎收口：MainActivity 拆分、日志事件化、清理残留。
- [x] **v3.1.0 全新 Logo 应用** ✨

### ⚠️ 目前的不足与挑战：
1.  **Logo 适配**：目前仅为静态图片替换，未完成完美的 Vector Drawable 矢量转化。
2.  **大任务中断**：在极少数低内存设备上，后台服务可能会被系统强制收回。
3.  **算法冗余**：MD5 计算目前针对大文件仍有优化空间（如采用抽样哈希）。

### 🗺️ 未来扩展路径 (致开发者)：
*   **多线程并发**：目前的替换是线性的，未来可以考虑在 Shizuku 层面实现多线程并发 Copy。
*   **云端配置**：支持从服务器拉取最新的游戏路径配置，无需更新 App 即可适配新版本游戏。
*   **AI 智能修复**：集成 AI 模块，自动分析并修复损坏的游戏资源文件。

---

## 💖 6. 价值观与使命

我们相信**真正的技术应该是温暖的**。
开源不只是共享代码，更是共享一种**创造的精神**。当你阅读完这份文档，我希望你产生的想法不只是"这软件不错"，而是：
> **"看起来这种逻辑我也能写，我也要去创造点什么！"**

在听风的世界里，我们尊重规则（道德与法律），但也崇拜自由（对技术的无尽探索）。让我们一起维护一个更好的 Android 生态。

---

## 💬 7. 与我联系

*   **微信**: Tf00798
*   **QQ**: 3353620663
*   **TG**: @TFGY999
*   **GitHub**: [AirFileEditor](https://github.com/lza6/AirFileEditor)

---
**听风改文件** —— 听见代码的声音，感受效率的轻盈。🌬️