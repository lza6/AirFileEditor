...
object PermissionChecker {
     ...
     */
    suspend fun checkPermissionAccess(
        ...
        withContext(Dispatchers.IO) {
            ...
            val message =
                when {
                    ...
                    androidVersion >= 30 -> {
                        if (isHarmonyOS) {
                            "HarmonyOS 已受限，请尝试 Root 或 Shizuku"
                        } else {
                            "Android 系统限制，推荐尝试高级模式"
                        }
                    }
                    else -> "建议尝试手动选择授权模式"
                }
     ...
     * 专门用于 Shizuku 测试的方法
     */
    private fun testDirectoryAccessShizuku(
        packageName: String,
        context: Context?,
    ): Boolean {
        return try {
    ...
    private fun testDirectoryAccess(packageName: String = PUBG_PACKAGE_NAME): Boolean {
        return try {
            ...
            if (hasRoot) {
                ...
                val listOldResult =
                    RootChecker.executeRootCommand(
                        "ls \"$testDataPath/${TEST_FILE_PREFIX}*.tmp\" 2>/dev/null",
                    )
                Log.d(TAG, "   旧的验证文件: ${listOldResult ?: "无"}")

                val cleanResult =
                    RootChecker.executeRootCommand(
                        "find \"$testDataPath\" -maxdepth 1 -name \"${TEST_FILE_PREFIX}*.tmp\" -delete 2>/dev/null && echo 'CLEANED' || echo 'NONE_TO_CLEAN'",
                    )
                Log.d(TAG, "   clean 输出: $cleanResult")

                // 验证清理结果
                val verifyCleanResult =
                    RootChecker.executeRootCommand(
                        "ls \"$testDataPath/${TEST_FILE_PREFIX}*.tmp\" 2>/dev/null",
                    )
