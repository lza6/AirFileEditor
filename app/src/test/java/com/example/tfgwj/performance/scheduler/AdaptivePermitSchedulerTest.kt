...
class AdaptivePermitSchedulerTest {
    ...
    @Test
    fun `scheduler adjusts permits based on IO speed`() = runTest {
        // Mock high IO speed, low memory usage
        every { PerformanceMonitor.getIOStats() } returns PerformanceMonitor.IOStats(
    ...
    @Test
    fun `scheduler reduces permits on high mmap fallback`() = runTest {
        every { PerformanceMonitor.getIOStats() } returns PerformanceMonitor.IOStats(
