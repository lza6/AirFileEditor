    ...
    Card(
        ...
        Column(
            ...
            Row(
                ...
                val (icon, color) = when (status.bestMode) {
                    PermissionChecker.AccessMode.ROOT,
                    PermissionChecker.AccessMode.NATIVE,
                    PermissionChecker.AccessMode.SHIZUKU -> Icons.Default.CheckCircle to Color(0xFF4CAF50)
                    else -> if (status.hasManageStorage) Icons.Default.Info to Color(0xFFFFC107) else Icons.Default.Error to Color(0xFFF44336)
                }
