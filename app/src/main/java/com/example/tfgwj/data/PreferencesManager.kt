...
class PreferencesManager(private val context: Context) {
    ...
    // 上次选择的文件夹路径
    val lastSelectedFolderPath: Flow<String?> =
    ...
    // 上次选择的主包路径
    val lastMainPackPath: Flow<String?> =
        context.dataStore.data
            .map { preferences ->
    ...
    // Shizuku授权状态
    val shizukuAuthorized: Flow<Boolean> =
        context.dataStore.data
            .map { preferences ->
    ...
    // 使用自动密码
    val useAutoPassword: Flow<Boolean> =
        context.dataStore.data
            .map { preferences ->
    ...
    // 上次解密密码
    val lastDecryptPassword: Flow<String?> =
        context.dataStore.data
            .map { preferences ->
    ...
    // 是否启用锁定时间
    val lockedTimeEnabled: Flow<Boolean> =
        context.dataStore.data
            .map { preferences ->
